...
auto sumarTodosFibonacci = [](const ListaEnlazada& lista) -> int {
    std::function<int(int)> sumaRecursiva;
    
    sumaRecursiva = [&lista, &sumaRecursiva](int index) -> int {
        if (index >= lista.getCapacity()) {
            return 0;
        }
        
        int fiboActual = sumaFiboElemento(lista, index);
        std::cout << "Fibonacci de lista[" << index << "] = " << fiboActual << std::endl;
        
        int sumaRestante = sumaRecursiva(index + 1);
        
        return fiboActual + sumaRestante;
    };
